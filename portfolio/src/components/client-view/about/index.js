...
import aboutMeImage from "../../../assets/programmer-silhouette.jpg";
...
export default function ClientAboutView({ data }) {
  ...
  const skillText = 'My Skills'

  return (
    <div className="max-w-screen-xl mt-24 mb-6 sm:mt-14 sm:mb-14 px-6 sm:px-8 lg:px-16 mx-auto" id="about">
      <div className="w-full flex">
        <AnimationWrapper className="rounded-lg w-full grid-flow-row grid grid-cols-1 sm:grid-cols-3 py-9 divide-y-2 sm:divide-y-0 sm:divide-x-2 divide-orange-500 bg-ehite-500 z-10">
            ...
            >
              <div className="flex m-0 w-40 sm:w-auto">
                <div className="flex flex-col">
                  <p className="text-[50px] text-orange-500 font-bold">
                    {infoItem.value}+
                  </p>
        ...
        <div className="flex flex-col justify-center items-center row-start-2 sm:row-start-1">
          <h1 className="leading-[70px] mb-4 text-3xl lg:text-4xl xl:text-5xl font-medium">
            {headingText.split(" ").map((item, index) => (
              <span
                className={`${index === 6 ? "text-orange-500" : "text-[#000]"}`}
              >
                {item}{" "}
              </span>
            ))}
          </h1>
          <p className="text-[#000] text-2xl mt-4 mb-8 font-bold">{data?.aboutme}</p>
          <h4 className="text-center text-3xl lg:text-4xl xl:text-5xl font-medium pt-10">
            {skillText.split(" ").map((item, index) => (
              <span
            ...
            {data?.skills.split(",").map((skill) => (
              ...
              >
                <motion.div 
                  ...
                  <button className="whitespace-nowrap overflow-hidden py-3 w-[160px] px-6 border-[2px] border-orange-500 bg-[#fff] text-[#000] font-semibold rounded-lg text-xl tracking-widest hover:shadow-orange-500 transition-all outline-none">
                    {skill}
                  </button>
                </motion.div>
              </motion.div>
