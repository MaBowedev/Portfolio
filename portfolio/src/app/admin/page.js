...
import { addData, getData, login,  updateData } from "@/service";
...
export default function AdminView() {
  ...
  async function handleSaveData() {
    ...
    if (response.success) {
      // Your code when response.success is true
      resetFormDatas();
      extractAllDatas();
    }
  }
  ...
  return (
    <>
      <div className="border-b border-gray-200">
        <nav role="tablist" className="-mb-05 flex justify-center space-x-6">
          {
            menuItems.map(item => (
              <button
                key={item.id}
